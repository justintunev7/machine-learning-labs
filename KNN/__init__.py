...
def five():
    ...
    KNN = KNNClassifier(labeltype ='classification',weight_type='inverse_distance', k=7, use_distance_weighting=False, regression=True)
...
def four():
    ...
    _, mse, labels = try_k(norm_train_data, norm_test_data, train_labels, test_labels, reg=True, use_distance_weighting=True)    
    plot_bar(mse, labels, xlabel="k", ylabel="MSE", title="House Price MSE by K value")
    ...
    _, mse, labels = try_k(norm_train_data, norm_test_data, train_labels, test_labels, reg=True, use_distance_weighting=True)    
    plot_bar(mse, labels, xlabel="k", ylabel="MSE", title="Magic Telescope MSE by K value")
...
def three():
    ...
    KNN = KNNClassifier(labeltype ='classification',k=3, use_distance_weighting=False, regression=True)
    ...
    _, mse, labels = try_k(norm_train_data, norm_test_data, train_labels, test_labels, reg=True)  
    plot_bar(mse, labels, xlabel="k", ylabel="MSE", title="Housing Price KNN MSE by K value")
...
def two():
    ...
    test_data, test_labels = setup_data("magic_telescope_test.arff")
    # # without normalizing
    KNN = KNNClassifier(labeltype ='classification',weight_type='inverse_distance', k=3, use_distance_weighting=False)
    ...
    accuracies, _, labels = try_k(norm_train_data, norm_test_data, train_labels, test_labels)
    plot_bar(accuracies, labels, xlabel="k", ylabel="Accuracy", title="Magic Telescope KNN Accuracy by K value")
...
def try_k(norm_train_data, norm_test_data, train_labels, test_labels, reg=False, use_distance_weighting=False):
    ...
    labels = []
    while k <= 15:
        KNN = KNNClassifier(labeltype ='classification',weight_type='inverse_distance', k=k, use_distance_weighting=use_distance_weighting, regression=reg)
        KNN.fit(norm_train_data,train_labels)
        pred = KNN.predict(norm_test_data)
        error, accuracy = KNN.score(norm_test_data,test_labels)
